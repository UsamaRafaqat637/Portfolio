...
export default function HeroSection() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 grid items-center max-w-6xl grid-cols-1 gap-10 px-6 mx-auto lg:px-12 md:grid-cols-2">
        ...
        <div className="flex flex-col gap-6 text-center md:text-left">
          <h2 className="text-3xl font-extrabold sm:text-4xl md:text-5xl">
            Hi, I&apos;m <span className="text-[#facc15]">Usama</span>
          </h2>
     ...
     {/* Sidebar - Right side for desktop */}
<nav className="fixed z-20 flex-col hidden gap-4 md:flex right-6 top-1/3">
  {navItems.map(({ id, label, Icon }, idx) => (
    ...
    >
      {/* Tooltip Label */}
      <span className="absolute right-14 opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 text-sm font-medium bg-[#0f1724]/80 backdrop-blur-sm text-[#facc15] px-3 py-1.5 rounded-md shadow-[0_0_10px_#facc15]/50 whitespace-nowrap">
        {label}
      </span>

      {/* Icon Button */}
      <button className="w-12 h-12 rounded-full bg-[#0f1724]/60 backdrop-blur-sm border border-white/5 flex items-center justify-center text-white transition-all duration-300 hover:text-[#facc15] hover:scale-110 hover:shadow-[0_0_15px_#facc15]">
        <Icon />
      </button>
    </a>
  ))}
      ...
      <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 z-20 flex md:hidden bg-[#0f1724]/60 backdrop-blur-md border border-white/10 rounded-full px-4 py-2 gap-3">
        {navItems.map(({ id, label, Icon }, idx) => (
          <a
            key={idx}
            href={`#${id}`}
            className="group flex flex-col items-center text-xs text-gray-300 hover:text-[#facc15] transition-all duration-300"
          >
            <Icon />
          </a>
        ))}
